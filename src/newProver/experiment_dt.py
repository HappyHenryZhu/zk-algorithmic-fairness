...
def sonic_experiment(size, aL, aR, aO, k, u, v, w, n, q, save=False, load=True):
    ...
    result = cmScheme.verify(list_of_c, *opens) and cmScheme.verify(list_of_c_o, *openOutsource) and t == field.sub(field.mul(r1, field.add(r2, s)), k) and s1 == s2
    print('result',result)

    print(f"list of commits(local): {list_of_c} \n" \
         f"opens : {opens}")


# list_of_c, w, wDash, rand_z, beta, list_of_fz, list_of_gamma, list_of_zSix, eval_zt, g_max
